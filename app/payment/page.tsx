...
export default function PaymentPage() {
  ...
  const handlePayment = async (planName: string, price: string, period: string) => {
    ...
    try {
      ...
      // Create Dodle payment session
      const response = await fetch('/api/payments/dodle', {
        method: 'POST',
        headers: {
      ...
      if (data.success) {
        // Redirect to Dodle payment page
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {plans.map((plan) => (
            ...
            >
              ...
              <button 
                onClick={() => handlePayment(plan.name, plan.price, plan.period)}
              ...
              >
                {processingPayment === plan.name ? 'Processing...' : 'Get Started'}
              </button>
            </div>
          ))}
        </div>
        ...
        <div className="mt-16 text-center text-sm text-muted-foreground">
          <p>💳 Secure payment powered by Dodle</p>
